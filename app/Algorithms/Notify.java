...
public class Notify {
		...
		public static void OnJoinOrQuit(String kind, ArrayList<Member> members) {
	        ...
	        for(Member member: members){
	        	...
	        	user_data.put("points", Integer.toString(user.points));
	        	m.add(user_data);
	        }
	        for(Member member: members){
	    ...
	    public static void OneOnAnswer(Member member){
	    	 ObjectNode event = Json.newObject();
		     event.put("kind", "answer");
		     event.put("button", member.button);
		     member.channel.write(event);
	    }
	    
	    public static void OnNoRightAnswer(ArrayList<Member> answered, GameState gamestate){
	    	ObjectNode event = Json.newObject();
	        event.put("kind", "message");
	        String string ="Няма познали!";
	        for(Member member: answered){
	        	string = string + " " + member.name + " отговори " + member.answer;
	        }
	        event.put("message", string);
	        for(Member member: gamestate.members){
